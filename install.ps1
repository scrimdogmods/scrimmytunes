...
# You can simply download and run it to have ScrimmyTunes installed for you.
...
# Where to put ScrimmyTunes by default.  Updated by repo detection.
# prolly should be param, but someone who cares about windows can code for it.
$Install_Dir = (pwd).Path + '\ScrimmyTunes\'
...
""
"This installer attempts to provide automatic install for ScrimmyTunes and dependencies."
...
$MB_RepoURL = "https://github.com/scrimdogmods/ScrimmyTunes.git"
...
$MB_Module_Dir=(pwd).Path + '\ScrimmyTunes'
...
if((Test-Path $MB_Reqs_File) -and (Test-Path $MB_Module_Dir) -and (Test-Path $MB_Git_Dir) ) {
    ...
    ""
    "ScrimmyTunes currently has three branches available."
    "  master - Stable ScrimmyTunes, least updates and may at times be out-of-date."
    "  review - Newer ScrimmyTunes, usually stable with less updates than the dev branch."
    "  dev    - The newest ScrimmyTunes, latest features and changes which may need testing."
    if($anybranch) {
...
"Using $PYTHON to install and run ScrimmyTunes..."
""
Invoke-Expression "$PYTHON -m pip install --upgrade -r requirements.txt" 

# -------------------------------------------------CONFIGURE THE BOT---------------------------------------------------
""
"ScrimmyTunes is almost ready to run, we just need to configure the bot."
...
if($iagree -ne "Y" -and $iagree -ne "y")
{
    ...
    "You must open a new command prompt before using run.bat to start the ScrimmyTunes."
    "ScrimmyTunes was installed to:"
    "  $Install_Dir"
    Return
}
