#!/bin/bash
#
# ScrimmyTunes and this file are provided under an MIT license. 
# Please see the LICENSE file for details.
#
# This file attempts to provide automatic install of ScrimmyTunes and dependencies on
# a variety of different Linux distros.
# 

#-----------------------------------------------Configs-----------------------------------------------#
ScrimmyTunesGitURL="https://github.com/scrimdogmods/ScrimmyTunes.git"
CloneDir="ScrimmyTunes"
VenvDir="ScrimmyTunesVenv"
InstallDir=""
ServiceName="ScrimmyTunes"
...
function list_supported() {
    ...
    echo ""
    echo "The ScrimmyTunes installer might have support for these flavors of Linux:"
    get_supported
    echo ""
    exit 0
}
...
function show_help() {
    ...
    echo "ScrimmyTunes Installer script usage:"
    echo "  $0 [OPTIONS]"
    echo ""
    ...
    echo "  --service   Install only the system service for ScrimmyTunes."
    ...
    echo "  --dir [PATH]    Directory into which ScrimmyTunes will be installed. Default is user Home directory."
    echo ""
    exit 0
}
...
function in_existing_repo() {
    # check the current working directory is a ScrimmyTunes repo clone.
    GitDir="${PWD}/.git"
    BotDir="${PWD}/ScrimmyTunes"
...
function in_venv() {
    ...
    # Assumes the current directory is a ScrimmyTunes clone.
...
function pull_ScrimmyTunes_git() {
    echo ""
    ...
    if in_existing_repo && [ "$InstallDir" == "" ]; then
        echo "Existing ScrimmyTunes repo detected."
        ...
        echo "Installer will attempt to create a new directory for ScrimmyTunes."
    fi
    ...
    echo ""
    echo "ScrimmyTunes currently has three branches available."
    echo "  master - An older ScrimmyTunes, for older discord.py. May not work without tweaks!"
    echo "  review - Newer ScrimmyTunes, usually stable with less updates than the dev branch."
    echo "  dev    - The newest ScrimmyTunes, latest features and changes which may need testing."
    ...
    "dev")
        ...
        git clone "${ScrimmyTunesGitURL}" "${CloneDir}" -b dev
        ;;
    "review")
        echo "Installing from 'review' branch..."
        git clone "${ScrimmyTunesGitURL}" "${CloneDir}" -b review
        ;;
    "master")
        echo "Installing from 'master' branch..."
        git clone "${ScrimmyTunesGitURL}" "${CloneDir}" -b master
        ;;
    *)
        if [ "$EnableUnlistedBranches" == "1" ] ; then
            echo "Installing from '${BRANCH}' branch..."
            git clone "${ScrimmyTunesGitURL}" "${CloneDir}" -b "$BRANCH"
        else
            exit_err "Unknown branch name given, install cannot continue."
        fi
        ;;
    esac
    cd "${CloneDir}" || exit_err "Fatal:  Could not change to ScrimmyTunes directory."
...
function issue_root_warning() {
    echo "Just like my opinion, but root and ScrimmyTunes shouldn't mix."
...
function generate_service_file() {
    ...
    cat << EOSF > "$1"
[Unit]
Description=scrimdogmods/ScrimmyTunes a discord.py bot that plays music.
...
# Replace with a path where ScrimmyTunes was cloned into.
...
function setup_as_service() {
    ...
    if ! in_existing_repo ; then
        if [ "$InstallDir" != "" ]; then
            ...
            echo "Please add the --dir option or install the ScrimmyTunes first."
            echo ""
            return 1
        fi
    fi
    ...
    Inst_Group="$(id -gn)"
    echo ""
    echo "The installer can also install ScrimmyTunes as a system service."
    echo "This starts the ScrimmyTunes at boot and restarts after failures."
    read -rp "Install the ScrimmyTunes system service? [N/y] " SERVICE
    case $SERVICE in
    [Yy]*)
        ...
        echo ""
        echo "Setting up ScrimmyTunes as a service named:  ${ServiceName}"
        ...
        if [ "$SKIP_ALL_SUDO" == "0" ] ; then
            ...
            echo ""
            echo "ScrimmyTunes will start automatically after the next reboot."
            read -rp "Would you like to start ScrimmyTunes now? [N/y]" StartService
...
# display preamble
cat << EOF
ScrimmyTunes Installer

ScrimmyTunes and this installer are provided under an MIT license.
...
This installer attempts to provide automatic install for ScrimmyTunes and dependency packages.
...
Wiki:
    https://scrimdogmods.github.io/ScrimmyTunes/
Discord:
...
if [ "$(id -u)" -eq "0" ] && [ "$INSTALL_BOT_BITS" == "1" ] ;  then
    ...
    echo ""
    echo -e "\e[1;37m\e[41m  Warning  \e[0m  You are using root and installing ScrimmyTunes."
    echo "        This can break python permissions and will create ScrimmyTunes files as root."
...
if [ "$InstallDir" == "" ] ; then
    cd "$(dirname "${BASH_SOURCE[0]}")" || { exit_err "Could not change directory for ScrimmyTunes installer."; }
fi

echo ""
if [ "${INSTALL_SYS_PKGS}${INSTALL_BOT_BITS}" == "11" ] ; then
    echo "Attempting to install required system packages & ScrimmyTunes software..."
else
    if [ "${INSTALL_SYS_PKGS}${INSTALL_BOT_BITS}" == "10" ] ; then
        echo "Attempting to install only required system packages..."
    else
        echo "Attempting to install only ScrimmyTunes and pip libraries..."
    fi
fi
echo ""
...
*"CentOS"* )
    ...
    *"CentOS "[2-6]* |*"CentOS 8."[0-5]* )
        ...
        echo "You should consider upgrading to the latest version to make installing ScrimmyTunes easier."
        exit 1
        ;;
...
if [ "$InstalledViaVenv" == "1" ] ; then
    echo ""
    echo "Notice:"
    echo "  This system required ScrimmyTunes to be installed inside a Python venv."
    echo "  Shell scripts included with ScrimmyTunes should detect and use the venv automatically."
