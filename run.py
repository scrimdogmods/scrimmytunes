...
from scrimmytunes.constants import (
    DEFAULT_LOGS_KEPT,
    DEFAULT_LOGS_ROTATE_FORMAT,
    MAXIMUM_LOGS_LIMIT,
)
from scrimmytunes.constants import VERSION as BOTVERSION
from scrimmytunes.exceptions import (
    HelpfulError,
    RestartCode,
    RestartSignal,
    TerminateSignal,
)
from scrimmytunes.utils import (
    rotate_log_files,
    set_logging_level,
...
log = logging.getLogger("scrimmytunes.launcher")
...
def req_check_deps() -> None:
    ...
    try:
        ...
        if discord.version_info.major < 2:
            log.critical(
                "This version of ScrimmyTunes requires a newer version of discord.py. "
...
def req_ensure_env() -> None:
    ...
    try:
        ...
        if not os.path.isdir("scrimmytunes"):
            raise RuntimeError('folder "scrimmytunes" not found')

        if not os.path.isfile("ScrimmyTunes/__init__.py"):
            raise RuntimeError("ScrimmyTunes folder is not a Python module")

        if not importlib.util.find_spec("scrimmytunes"):
            raise RuntimeError("ScrimmyTunes module is not importable")
    except RuntimeError as e:
        log.critical("Failed environment check, %s", e)
        bugger_off()

    try:
        os.mkdir("ScrimmyTunes-test-folder")
    except (
        OSError,
        FileExistsError,
        PermissionError,
        ...
        shutil.rmtree("ScrimmyTunes-test-folder", True)
    ...
    if not ffmpeg_bin:
        log.critical(
            "ScrimmyTunes could not locate FFmpeg binary in your environment.\n"
            "Please install FFmpeg so it is available in your environment PATH variable."
        )
        if sys.platform.startswith("win"):
            log.info(
                "On Windows, you can add a pre-compiled EXE to the ScrimmyTunes `bin` folder,\n"
...
def opt_check_updates() -> None:
    ...
    """
    log.info("\nChecking for updates to ScrimmyTunes or dependencies...")
    needs_update = False
    if GIT.works():
        ...
        if commit_at and commit_to:
            log.warning(
                "ScrimmyTunes updates are available through `git` command.\n"
            ...
            log.info("No ScrimmyTunes updates available via `git` command.")
    else:
        log.warning(
...
def parse_cli_args() -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(
        ...
        Available via Github:
          https://github.com/Just-Some-Bots/ScrimmyTunes
        """
        ),
    ...
    ap.add_argument(
        ...
        dest="show_version",
        action="store_true",
        help="Print the ScrimmyTunes version information and exit.",
    )
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="Disable ScrimmyTunes from trying to install dependencies when it cannot import them.",
    )
    ...
    if args.show_version:
        print(f"Just-Some-Bots/ScrimmyTunes\nVersion:  {BOTVERSION}\n")
        sys.exit(0)
...
def respawn_bot_process() -> None:
    ...
    if os.name == "nt":
        ...
        print("Opened a new ScrimmyTunes instance. This terminal can be safely closed!")
        sys.exit(0)
    else:
...
def set_console_title() -> None:
    ...
    try:
        sys.stdout.write(f"\x1b]2;ScrimmyTunes {BOTVERSION}\x07")
...
def main() -> None:
    """
    All of the ScrimmyTunes starts here.
    """
    ...
    log.info("Loading ScrimmyTunes version:  %s", BOTVERSION)
    ...
    if run_py_dir != os.getcwd():
        # if not, verify ScrimmyTunes and .git folders exists and change directory.
        run_mb_dir = pathlib.Path(run_py_dir).joinpath("scrimmytunes")
        ...
        if run_mb_dir.is_dir() and run_git_dir.is_dir():
            ...
            log.critical(
                ...
                " and we could not locate `ScrimmyTunes` and `.git` folders to verify"
                " a new directory location."
            )
            log.error(
                "For best results, start `run.py` from the same folder you cloned ScrimmyTunes into.\n"
    ...
    while True:
        ...
        m = None
        try:
            # Prevent re-import of ScrimmyTunes
            if "scrimmytunes" not in dir():
                from scrimmytunes.bot import (  # pylint: disable=import-outside-toplevel
                    ScrimmyTunes,
                )
            ...
            m = ScrimmyTunes(use_certifi=use_certifi)
            ...
            # let the ScrimmyTunes run free!
            event_loop.run_until_complete(m.run_ScrimmyTunes())
            retries = 0
            ...
            if cli_args.no_install_deps:
                helpfulerr = HelpfulError(
                    preface="Cannot start ScrimmyTunes due to an error!",
                    issue=(
                        f"Error: {str(e)}\n"
                        "This is an error importing ScrimmyTunes or a dependency package."
                    ),
                    solution=(
                        "You need to manually install dependency packages via pip.\n"
                        "Or launch without `--no-install-deps` and ScrimmyTunes will try to install them for you."
                    ),
                    footnote=(
                        "You have the `--no-install-deps` option set."
                        "Normally ScrimmyTunes attempts "
                    ),
                )
            ...
            if not PIP.works():
                log.critical(
                    "ScrimmyTunes could not import dependency modules and we cannot run `pip` automatically!\n"
                ...
                log.warning(
                    ...
                    " - Check installed packages are accessible to the user running ScrimmyTunes",
            ...
            if not tried_requirementstxt:
                ...
                log.info(
                    "Attempting to install ScrimmyTunes dependency packages automatically...\n"
                )
                ...
                if pip_exit_code:
                    ...
                    dep_error = HelpfulError(
                        preface="ScrimmyTunes dependencies may not be installed!",
                        issue="We didn't get a clean exit code from `pip` install.",
                        solution=(
                            "You will need to manually install dependency packages.\n"
                            "ScrimmyTunes tries to use the following command, so modify as needed:\n"
                            "  pip install -U -r ./requirements.txt"
                        ),
                        footnote="You can also ask for help in ScrimmyTunes support server:  https://discord.gg/bots",
                    )
            ...
            log.error(
                "ScrimmyTunes got an ImportError after trying to install packages. ScrimmyTunes must exit..."
            )
            ...
            if e.get_name() == "RESTART_SOFT":
                log.info("ScrimmyTunes is doing a soft restart...")
                retries = 1
                continue

            log.info("ScrimmyTunes is doing a full process restart...")
            exit_signal = e
            retries = 1
            break
