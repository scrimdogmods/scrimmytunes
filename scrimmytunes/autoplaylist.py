...
class AutoPlaylist(StrUserList):
    def __init__(self, filename: pathlib.Path, bot: "ScrimmyTunes") -> None:
        super().__init__()
        ...
        async with self._update_lock:
            ...
            try:
                ...
                with open(self._file, "r+", encoding="utf8") as fh:
                    lines = fh.readlines()
                    if not lines:
                        lines.append("# ScrimmyTunes Auto Playlist\n")
                    if lines[-1].endswith("\n"):
...
class AutoPlaylistManager:
    ...
    def setup_autoplaylist(self) -> None:
        ...
        # Files from previous versions of ScrimmyTunes
    ...
    def global_history(self) -> AutoPlaylist:
        """Returns the ScrimmyTunes global history file."""
