...
from .constants import (
    ...
    SCRIMMYTUNES_USER_AGENT_AIOHTTP,
)
...
# Channels that ScrimmyTunes Can message.
...
# Voice Channels that ScrimmyTunes Can connect to.
...
class ScrimmyTunes(discord.Client):
    def __init__(
        self,
        ...
        log.info("Initializing Scrimmy Tunes %s", BOTVERSION)
        load_opus_lib()
    ...
    async def setup_hook(self) -> None:
        ...
        self.http.user_agent = SCRIMMYTUNES_USER_AGENT_AIOHTTP
        if self.use_certifi:
    ...
    async def _test_network_via_ping(self, ping_target: str) -> int:
        ...
        try:
            ...
            log.error(
                "Scrimmy Tunes could not locate a `ping` command path.  Will attempt to use HTTP ping instead."
                "\nScrimmy Tunes tried the following command:   %s"
            ...
            log.error(
                "Scrimmy Tunes was denied permission to execute the `ping` command.  Will attempt to use HTTP ping instead."
                "\nScrimmy Tunes tried the following command:   %s"
                "\nYou should enable ping in your system or container environment for best results."
                "\nAlternatively disable network checking via config.",
                " ".join(ping_cmd),
    ...
    def on_network_up(self) -> None:
        """
        Event called by ScrimmyTunes when it detects network returned from outage.
        """
        log.info("Scrimmy Tunes detected network is available again.")
    ...
    def on_network_down(self) -> None:
        """
        Event called by ScrimmyTunes when it detects network outage.
        """
        log.info("Scrimmy Tunes detected a network outage.")
        for gid, player in self.players.items():
        ...
        for guild, channel in channel_map.items():
            ...
            if channel and isinstance(
                ...
                if player:
                    ...
                    log.debug("Scrimmy Tunes will make a new MusicPlayer now...")
                    try:
                        player = await self.get_player(
                            channel,
                            create=True,
    ...
    async def get_voice_client(self, channel: VoiceableChannel) -> discord.VoiceClient:
        ...
        :raises: ScrimmyTunes.exceptions.PermissionsError
            If ScrimmyTunes does not have permissions required to join or speak in the `channel`.
        """
        if not isinstance(channel, (discord.VoiceChannel, discord.StageChannel)):
            raise TypeError("Channel passed must be a voice channel")

        # Check if ScrimmyTunes has required permissions to join in channel.
        chperms = channel.permissions_for(channel.guild.me)
        if not chperms.connect:
            log.error(
                "Scrimmy Tunes does not have permission to Connect in channel:  %s",
                channel.name,
            )
            raise exceptions.PermissionsError(
                f"Scrimmy Tunes does not have permission to Connect in channel:  `{channel.name}`",
                expire_in=30,
            )
        if not chperms.speak:
            log.error(
                "Scrimmy Tunes does not have permission to Speak in channel:  %s",
                channel.name,
            )
            raise exceptions.PermissionsError(
                f"Scrimmy Tunes does not have permission to Speak in channel:  `{channel.name}`",
                expire_in=30,
            )
        ...
        for attempt in range(1, (VOICE_CLIENT_MAX_RETRY_CONNECT + 1)):
            ...
            if timeout > max_timeout:
                log.critical(
                    "Scrimmy Tunes is unable to connect to the channel right now:  %s",
                    channel,
                )
                raise exceptions.CommandError(
                    "Scrimmy Tunes could not connect to the channel. Try again later, or restart the bot if this continues."
                )
            ...
            try:
                ...
                log.voicedebug(  # type: ignore[attr-defined]
                    "Scrimmy Tunes has a VoiceClient now..."
                )
                break
                ...
                log.exception(
                    "Scrimmy Tunes VoiceClient connection attempt was cancelled. No retry."
                )
                raise exceptions.CommandError(
                    "Scrimmy Tunes connection to voice was cancelled. This is odd. Maybe restart?"
                ) from e
    ...
    async def disconnect_voice_client(self, guild: discord.Guild) -> None:
        ...
        for vc in self.voice_clients:
            if not isinstance(vc, discord.VoiceClient):
                log.debug(
                    "Scrimmy Tunes has a VoiceProtocol that is not a VoiceClient. Disconnecting anyway..."
                )
                try:
    ...
    async def disconnect_all_voice_clients(self) -> None:
        ...
        for vc in self.voice_clients:
            if isinstance(vc, discord.VoiceClient):
                ...
                log.warning(
                    "Scrimmy Tunes.voice_clients list contains a non-VoiceClient object?\n"
    ...
    async def get_player(
        ...
        *,
        ...
        :raises:  ScrimmyTunes.exceptions.PermissionsError
            If ScrimmyTunes is not permitted to join the given `channel`.
        """
        guild = channel.guild
    ...
    def _init_player(
        ...
        """
        Connect a brand-new MusicPlayer instance with the ScrimmyTunes event
    ...
    async def on_player_play(self, player: MusicPlayer, entry: EntryTypes) -> None:
        ...
        Simplified to only display the song title.
        """
        ...
        message = f"Now playing: {entry.title}"

        if self.config.dm_nowplaying and entry.author:
            await self.safe_send_message(entry.author, message)
            return
        ...
        if self.config.nowplaying_channels:
            for ch_id in self.config.nowplaying_channels:
                ch = self.get_channel(ch_id)
                if isinstance(ch, discord.abc.Messageable) and ch.guild == guild:
                    np_channel = ch
                    break

        if not np_channel:
            last_msg = self.server_data[guild.id].last_np_msg
            if last_msg:
                np_channel = last_msg.channel
    ...
    async def on_player_finished_playing(self, player: MusicPlayer, **_: Any) -> None:
        ...
        if (
            ...
            while player.autoplaylist:
                ...
                except exceptions.ScrimmyTunesException:
                    log.exception(
                        "Scrimmy Tunes needs to stop the autoplaylist extraction and bail."
                    )
                    return
                except Exception:  # pylint: disable=broad-exception-caught
                    log.exception(
                        "Scrimmy Tunes got an unhandled exception in player finished event."
                    )
                    break
    ...
    async def on_os_signal(
        ...
        the start of run_ScrimmyTunes().
        This allows ScrimmyTunes to handle external signals and triggering a clean
        shutdown of ScrimmyTunes in response to them.

        It essentially just calls logout, and the rest of ScrimmyTunes tear-down is
        finished up in `ScrimmyTunes.run_ScrimmyTunes()` instead.
        ...
        try:
            if self and not self.logout_called:
                log.info("Disconnecting and closing down Scrimmy Tunes...")
                await self.logout()
        except Exception as e:
            log.exception("Exception thrown while handling interrupt signal!")
            raise KeyboardInterrupt() from e

    async def run_ScrimmyTunes(self) -> None:
        """
        This method is to be used in an event loop to start the ScrimmyTunes.
        ...
        try:
            ...
            log.info("Scrimmy Tunes is now doing shutdown steps...")
            ...
            raise exceptions.HelpfulError(
                ...
                issue="Scrimmy Tunes cannot login to discord, is your token correct?",
            ...
            for task in asyncio.all_tasks(loop=self.loop):
                # Don't cancel run_ScrimmyTunes task, we need it to finish cleaning.
    ...
    async def logout(self) -> None:
        """
        Disconnect all voice clients and signal ScrimmyTunes to close it's connections to discord.
        """
    ...
    async def on_resumed(self) -> None:
        ...
        """
        log.info("ScrimmyTunes resumed a session with discord.")
        await self._auto_join_channels(from_resume=True)

    async def on_ready(self) -> None:
        """
        Event called by discord.py typically when ScrimmyTunes has finished login.
    ...
    async def _on_ready_once(self) -> None:
        ...
        """
        mute_discord_console_log()
        log.debug("Logged in, now getting Scrimmy Tunes ready...")
    ...
    def _gen_embed(self) -> discord.Embed:
        ...
        e.set_footer(
            text=self.config.footer_text, icon_url="http://afog.site/scrimmytunes/img/st.png"
        )

        # TODO: handle this part when EmbedResponse get handled.
        author_name = "Scrimmy Tunes"
        avatar_url = None
        if self.user:
        ...
        e.set_author(
            name=author_name,
            url="http://afog.site/scrimmytunes",
            icon_url="http://afog.site/scrimmytunes/img/st.png",
        )
        return e
    ...
    def _do_song_blocklist_check(self, song_subject: str) -> None:
        ...
        :raises: ScrimmyTunes.exceptions.CommandError
    ...
    async def handle_vc_inactivity(self, guild: discord.Guild) -> None:
        """
        Manage a server-specific event timer when ScrimmyTunes's voice channel becomes idle,
        ...
        if is_emoji:
            desc = (
                ...
                + self.str.get(
                    ...
                    "For further help, see http://afog.site/scrimmytunes",
                ).format(
                    example_cmd=(
    ...
    async def cmd_blockuser(
        ...
        if not user_mentions:
            raise exceptions.CommandError(
                "Scrimmy Tunes could not find the user(s) you specified.",
                expire_in=20,
            )
    ...
    async def cmd_follow(
        ...
        ScrimmyTunes will automatically follow a user when they change channels.
        """
        # If ScrimmyTunes is already following a user, either change user or un-follow.
        ...
        if author.id == self.config.owner_id and user_mentions:
            ...
            if not isinstance(m, discord.Member):
                raise exceptions.CommandError(
                    "Scrimmy Tunes cannot follow a user that is not a member of the server.",
                    expire_in=30,
                )
            bind_to_member = m
    ...
    async def cmd_config(
        ...
        This command allows management of ScrimmyTunes config options file.
        """
    ...
    async def cmd_queue(
        ...
        if not q_msg:
            ...
            raise exceptions.CommandError(
                "Try that again. Scrimmy Tunes couldn't make or get a reference to the queue message.  If the issue persists, file a bug report."
            )
    ...
    async def cmd_disconnect(self, guild: discord.Guild) -> CommandResponse:
        ...
        for vc in self.voice_clients:
            if not hasattr(vc.channel, "guild"):
                log.warning(
                    "Scrimmy Tunes found a %s with no guild!  This could be a problem.",
                    type(vc),
                )
                continue
    ...
    async def cmd_checkupdates(self, channel: MessageableChannel) -> CommandResponse:
        ...
        Display the current bot version and check for updates to ScrimmyTunes or dependencies.
        """
        git_status = ""
        pip_status = ""
        updates = False
        ...
        if updates:
            header = "There are updates for Scrimmy Tunes available for download."
        else:
            header = "Scrimmy Tunes is totally up-to-date!"
    ...
    async def cmd_uptime(self) -> CommandResponse:
        ...
        Displays the ScrimmyTunes uptime, since last start/restart.
        """
    ...
    async def cmd_latency(self, guild: discord.Guild) -> CommandResponse:
        ...
        Prints the latency info available to ScrimmyTunes.
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        self_mention = "<@ScrimmyTunes>"  # placeholder
        if self.user:
    ...
    async def on_inactivity_timeout_expired(
        ...
        """
        A generic event called by ScrimmyTunes when configured channel or player
    ...
    async def on_connect(self) -> None:
        ...
        if self.init_ok:
            log.info("Scrimmy Tunes has become connected.")

    async def on_disconnect(self) -> None:
        """Event called by discord.py any time bot is disconnected, or fails to connect."""
        log.info("Scrimmy Tunes has become disconnected.")
    ...
    async def _handle_api_disconnect(self, before: discord.VoiceState) -> bool:
        """
        Method called from on_voice_state_update when ScrimmyTunes is disconnected from voice.
        """
    ...
    async def on_guild_channel_update(
        ...
        # The chat delay is not currently respected by ScrimmyTunes. Is this a problem?
