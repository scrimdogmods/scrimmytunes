...
from .exceptions import ExtractionError, ScrimmyTunesException
...
class Downloader:
    def __init__(self, bot: "ScrimmyTunes") -> None:
        ...
        if bot.config.cookies_path.is_file():
            log.info(
                "ScrimmyTunes will use cookies for yt-dlp from:  %s",
        ...
        if bot.config.ytdlp_use_oauth2:
            ...
            if oauth_spec is not None:
                log.warning(
                    ...
                    "This may cause ScrimmyTunes to not close completely, or hang pending authorization!\n"
                    "To close ScrimmyTunes, you must manually Kill the ScrimmyTunes process!\n"
    ...
    async def extract_info(
        ...
        :raises: ScrimmyTunes.exceptions.ScrimmyTunesError
            if event loop is closed and cannot be used for extraction.

        :raises: ScrimmyTunes.exceptions.ExtractionError
            for errors in ScrimmyTunes's internal filtering and pre-processing of extraction queries.

        :raises: ScrimmyTunes.exceptions.SpotifyError
            for issues with ScrimmyTunes's Spotify API request and data handling.
    ...
    async def _filtered_extract_info(
        ...
        if (self.bot.loop and self.bot.loop.is_closed()) or not self.bot.loop:
            ...
            raise ScrimmyTunesException("Cannot continue extraction, event loop is closed.")
    ...
    def _return_local_media(self, song_subject: str) -> "YtdlpResponseDict":
        ...
        if not local_file_path.is_file():
            raise ScrimmyTunesException("The local media file could not be found.")
        ...
        return YtdlpResponseDict(
            {
                ...
                "extractor": "local:ScrimmyTunes",
                "extractor_key": "LocalMediaScrimmyTunes",
