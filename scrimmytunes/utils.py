...
def setup_loggers() -> None:
    """set up all logging handlers for ScrimmyTunes and discord.py"""
    if len(logging.getLogger("ScrimmyTunes").handlers) > 1:
        log.debug("Skipping logger setup, already set up")
        return

    # Do some pre-flight checking...
    log_file = pathlib.Path(DEFAULT_SCRIMMYTUNES_LOG_FILE)
    ...
    logger = logging.getLogger("scrimmytunes")
    ...
    # Setup logging to file for ScrimmyTunes.
    try:
    ...
    # Setup logging to console for ScrimmyTunes, handle missing colorlog gracefully.
...
def set_logging_level(level: int, override: bool = False) -> None:
    """
    Sets the logging level for ScrimmyTunes and discord.py loggers.
...
def shutdown_loggers() -> None:
    """Removes all ScrimmyTunes and discord log handlers"""
    if not hasattr(logging, "_mb_logs_open"):
        return

    # This is the last log line of the logger session.
    log.info("ScrimmyTunes loggers have been called to shutdown.")
...
def rotate_log_files(max_kept: int = -1, date_fmt: str = "") -> None:
    ...
    # Rotate ScrimmyTunes logs
    logfile = pathlib.Path(DEFAULT_SCRIMMYTUNES_LOG_FILE)
    ...
    # Clean up old, out-of-limits, ScrimmyTunes log files
...
def owner_only(func: Callable[..., Any]) -> Any:
    ...
    the Owner ID which ScrimmyTunes has determined either via Config or
    Discord AppInfo.
    """

    @wraps(func)
    async def wrapper(self: "ScrimmyTunes", *args: Any, **kwargs: Any) -> Any:
...
def dev_only(func: Callable[..., Any]) -> Any:
    ...
    it decorates.
    This is then checked in ScrimmyTunes.on_message to ensure the protected
...
def count_members_in_voice(  # pylint: disable=dangerous-default-value
    ...
    By default it excludes all deaf users, all bots, and the ScrimmyTunes client itself.
