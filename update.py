...
def update_deps() -> None:
    """
    Tries to upgrade ScrimmyTunes dependencies using pip module.
...
def update_ffmpeg() -> None:
    ...
    if (
        ...
        do_dl = yes_or_no_input(
            "Should we update the ScrimmyTunes bundled ffmpeg executables? [Y/n]"
        )
        if do_dl:
            dl_windows_ffmpeg()
        ...
        print(
            "We detected FFmpeg installed but it is not the exe bundled with ScrimmyTunes.\n"
...
def finalize() -> None:
    ...
    try:
        from scrimmytunes.constants import (  # pylint: disable=import-outside-toplevel
            VERSION,
        )

        print(f"The current ScrimmyTunes version is:  {VERSION}")
    except ImportError:
        print(
...
def main() -> None:
    ...
    if sys.platform.startswith("win"):
        ...
        print(
            f"Adding ScrimmyTunes bin folder to environment path for this run:  {bin_path}",
        )
    ...
    if status_unclean.strip():
        ...
        print(
            ...
            f"{status_unclean}\n"
            "To update ScrimmyTunes source code, you must first remove modifications made to the above source files.\n"
        ...
        hard_reset = yes_or_no_input(
            ...
            "Would you like to reset the Source code, to allow ScrimmyTunes to update?"
        )
        if hard_reset:
            run_or_raise_error(
                [git_bin, "reset", "--hard"],
                "Could not hard reset the directory to a clean state.\n"
                "You will need to manually reset the local git repository, or make a new clone of ScrimmyTunes.",
            )
        else:
