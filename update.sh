...
# make sure we're in ScrimmyTunes directory...
cd "$(dirname "${BASH_SOURCE[0]}")" || { echo "Could not change directory to ScrimmyTunes."; exit 1; }

# provides an exit that also deactivates venv.
function do_exit() {
    if [ "${VIRTUAL_ENV}" != "" ] ; then
        echo "Leaving ScrimmyTunes Venv..."
        deactivate
    fi
    exit "$1"
}
...
if [ -f "../bin/activate" ] ; then
    echo "Detected ScrimmyTunes Venv & Loading it..."
...
if [[ "$VerGood" == "0" ]]; then
    echo "Python 3.8.7 or higher is required to update ScrimmyTunes."
    do_exit 1
fi

echo "Using '${Python_Bin}' to update ScrimmyTunes..."
